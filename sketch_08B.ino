// Example 08B: Arduino Networked Lamp, 20181028
...
int val = 0; // to store the value coming from sensor
...
void loop() {
  ...
  Serial.println(val);      // print the value of the serial port
  ...
  if (Serial.available() > 0) {
    ...
    if (inByte == '#') {
      ...
      // now we have 3 numbers stored as hex. We need to decode them into 3 bytes
      // r, g, and b.
  ...
  if (state == 1) {       // if the lamp is on
    // turn the leds on at the colour sent by the computer
    analogWrite(R_LED, r);
    analogWrite(G_LED, g);
    analogWrite(B_LED, b); 
  } else {
